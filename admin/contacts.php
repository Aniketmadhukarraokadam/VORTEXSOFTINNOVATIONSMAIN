...
if ($db) {
    try {
        ...
        if (isset($_GET['delete'])) {
            $did = (int)$_GET['delete'];
            $db->prepare("DELETE FROM contact_inquiries WHERE id = :id")->execute([':id' => $did]);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['note'])) {
            ...
            header("Location: contacts.php?view=$view_id&saved=1");
            exit;
        }
        ...
        $where  = "WHERE 1=1";
        $params = [];
        if ($filter === 'new' || $filter === 'unread') { $where .= " AND is_read=0"; }
        if ($filter === 'replied')                    { $where .= " AND is_replied=1"; }
        if ($search) {
            $where .= " AND (name LIKE :q OR email LIKE :q2 OR service LIKE :q3 OR message LIKE :q4)";
...
<title>Inquiries — Vortexsoft Admin Panel</title>
...
<main class="admin-main">
  ...
  <div class="alert alert-success mb-4" style="border-radius:12px;"><i class="fas fa-check-circle me-2"></i> Inquiry deleted successfully.</div>
  <?php endif; ?>
      ...
      <div class="col-12 mt-2">
        <form method="POST" action="contacts.php?view=<?= $view['id'] ?>">
          ...
          <div class="mt-3 d-flex gap-3 align-items-center flex-wrap">
            ...
            <a href="contacts.php?delete=<?= $view['id'] ?>" class="btn action-btn-delete" style="border-radius:8px;font-size:13px;font-weight:700;padding:10px 22px;" onclick="return confirm('Delete this inquiry permanently?');"><i class="fas fa-trash me-1"></i> Delete</a>
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="table-card">
    ...
    <div style="overflow-x:auto;">
      <table>
        <thead><tr><th>#</th><th>Name</th><th>Email</th><th>Service</th><th>Phone</th><th>Received</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
          ...
          <tr style="<?= !$c['is_read'] ? 'background:#fffbf5;' : '' ?>">
            ...
            <td>
              <a href="contacts.php?view=<?= $c['id'] ?>" class="action-btn"><i class="fas fa-eye"></i> View</a>
              <a href="contacts.php?delete=<?= $c['id'] ?>" class="action-btn action-btn-delete" onclick="return confirm('Delete this inquiry?');"><i class="fas fa-trash"></i></a>
            </td>
          </tr>
          <?php endforeach; ?>
          <?php endif; ?>
