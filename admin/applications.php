...
if ($db) {
    try {
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['status_update'])) {
            ...
            header("Location: applications.php?view=$view_id&updated=1"); exit;
        }
        ...
        if ($search) { $where.=" AND (applicant_name LIKE :q OR email LIKE :q2 OR job_title LIKE :q3 OR current_location LIKE :q4)"; $params[':q']=$params[':q2']=$params[':q3']=$params[':q4']='%'.$search.'%'; }

        $total_count = (int)$db->query("SELECT COUNT(*) FROM job_applications $where")->fetchColumn();
        $pg = paginate($total_count, ITEMS_PER_PAGE, $page);

        $stmt = $db->prepare("SELECT id,applicant_name,email,phone,job_title,experience_years,status,created_at,resume_path FROM job_applications $where ORDER BY created_at DESC LIMIT :l OFFSET :o");
...
<main class="admin-main">
  ...
  <div class="detail-card">
    <div class="row gy-3">
      ...
      <div class="col-md-6"><div style="font-size:12px;font-weight:700;text-transform:uppercase;color:#94a3b8;margin-bottom:4px;">Resume File</div>
        <?php if($view['resume_filename']): ?>
        <a href="<?= htmlspecialchars($view['resume_path']) ?>" target="_blank" class="btn btn-sm" style="background:#1C2280;color:#fff;border-radius:8px;font-weight:600;"><i class="fas fa-download me-1"></i> Download <?= htmlspecialchars($view['resume_filename']) ?></a>
  ...
  <div class="table-card">
    ...
    <div style="overflow-x:auto;">
      <table>
        <thead><tr><th>#</th><th>Applicant</th><th>Position</th><th>Experience</th><th>Resume</th><th>Applied</th><th>Status</th><th>Action</th></tr></thead>
        <tbody>
          <?php if(empty($apps)): ?>
          ...
          <tr>
            ...
            <td>
              ...
              <a href="<?= htmlspecialchars($a['resume_path']) ?>" target="_blank" style="font-size:12px;color:#CC2228;font-weight:700;"><i class="fas fa-file-pdf me-1"></i> Resume</a>
            ...
            <td><a href="applications.php?view=<?= $a['id'] ?>" class="action-btn"><i class="fas fa-eye"></i> View</a></td>
          </tr>
          <?php endforeach; ?>
          <?php endif; ?>
        </tbody>
